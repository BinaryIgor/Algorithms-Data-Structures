...
public final class RedBlackTree<T extends Comparable<T>> implements Tree<T> {
	...
	public void insert(T data) throws Exception {
		...
		this.root.fill(insert(this.root, node));
		if (hasGrandParent(node)) {
			fixViolations(node);
		}
	}

	private ColoredBinaryNode<T> insert(Potential<ColoredBinaryNode<T>> root,
	...
	private boolean hasGrandParent(ColoredBinaryNode<T> node) throws Exception {
		return node.parent().isFilled() && node.parent().value().parent().isFilled();
	}
	...
	private void fixViolations(ColoredBinaryNode<T> node) throws Exception {
		while (shouldFixViolation(node)) {
			...
			if (grandParent.left().isFilled() && parent.equals(grandParent.left().value())) {
	...
	private boolean shouldFixViolation(ColoredBinaryNode<T> node) throws Exception {
		return !node.equals(this.root.value()) && node.color().equals(RED)
				&& node.parent().value().color().equals(RED);
	}

	private ColoredBinaryNode<T> fixLeftChildViolation(ColoredBinaryNode<T> grandParent,
			ColoredBinaryNode<T> parent, ColoredBinaryNode<T> node) throws Exception {
		...
		if (isRed(uncle)) {
			...
			if (parent.right().isFilled() && node.equals(parent.right().value())) {
				rotateLeft(parent);
				node = parent;
	...
	private ColoredBinaryNode<T> fixRightChildViolation(ColoredBinaryNode<T> grandParent,
			ColoredBinaryNode<T> parent, ColoredBinaryNode<T> node) throws Exception {
		...
		if (isRed(uncle)) {
			...
			if (parent.left().isFilled() && node.equals(parent.left().value())) {
				rotateRight(parent);
				node = parent;
				parent = node.parent().value();
			}
	...
	private void rotateRight(ColoredBinaryNode<T> node) {
		...
		if (!leftNode.parent().isFilled()) {
			this.root.fill(leftNode);
		} else if (node.parent().value().left().isFilled()
				&& node.equals(node.parent().value().left().value())) {
	...
	private void rotateLeft(ColoredBinaryNode<T> node) {
		...
		if (!rightNode.parent().isFilled()) {
			this.root.fill(rightNode);
		} else if (node.parent().value().left().isFilled()
				&& node.equals(node.parent().value().left().value())) {
	...
	public void delete(T data) {
		try {
			Potential<ColoredBinaryNode<T>> deleted = deleted(new Potential<>(this.root).value(),
					data, true);
	...
	private Potential<ColoredBinaryNode<T>> deleted(Potential<ColoredBinaryNode<T>> root, T data,
			boolean caseOne) throws Exception {
		if (!root.isFilled()) {
			return root;
		}
		ColoredBinaryNode<T> rootValue = root.value();
	...
	private void deleteCaseTwo(ColoredBinaryNode<T> node) {
		...
		if (isRed(sibling)) {
			...
			if (node.parent().value().left().isFilled()
					&& node.equals(node.parent().value().left().value())) {
	...
	private void deleteCaseThree(ColoredBinaryNode<T> node) {
		...
		Potential<ColoredBinaryNode<T>> sibling = sibling(node);
		boolean caseOne = node.parent().value().color().equals(BLACK) && sibling.isFilled()
				&& (sibling.value().color().equals(BLACK))
				&& (!sibling.value().left().isFilled()
						|| sibling.value().left().value().color().equals(BLACK))
				&& (!sibling.value().right().isFilled()
						|| sibling.value().right().value().color().equals(BLACK));
		if (caseOne) {
	...
	private void deleteCaseFour(ColoredBinaryNode<T> node) {
		...
		Potential<ColoredBinaryNode<T>> sibling = sibling(node);
		boolean end = isRed(node.parent()) && sibling.isFilled()
				&& (sibling.value().color().equals(BLACK))
				&& (!sibling.value().left().isFilled()
						|| sibling.value().left().value().color().equals(BLACK))
				&& (!sibling.value().right().isFilled()
	...
	private void deleteCaseFive(ColoredBinaryNode<T> node) {
		...
		Potential<ColoredBinaryNode<T>> sibling = sibling(node);
		if (sibling.isFilled() && sibling.value().color().equals(BLACK)) {
	...
	private boolean isLeftChild(ColoredBinaryNode<T> node) {
		return node.parent().value().left().isFilled()
				&& node.parent().value().left().value().equals(node);
	}

	private boolean isRightChild(ColoredBinaryNode<T> node) {
		return node.parent().value().right().isFilled()
				&& node.parent().value().right().value().equals(node);
	}

	private boolean isBlack(Potential<ColoredBinaryNode<T>> node) {
		return !node.isFilled() || node.value().color().equals(BLACK);
	}

	private boolean isRed(Potential<ColoredBinaryNode<T>> node) {
		return node.isFilled() && node.value().color().equals(RED);
	}
	...
	private Potential<ColoredBinaryNode<T>> sibling(ColoredBinaryNode<T> node) {
		...
		} else if (node.parent().value().right().isFilled()
				&& !node.parent().value().right().value().equals(node)) {

...
public final class LinearProbingArrayHashTable<K, V> implements HashTable<K, V> {
    ...
    public HashTable<K, V> save(K key, V value) {
        ...
        if (previous == null || previous.key().equals(key)) {
            ...
            this.entries[linearProbedHashedKey(hashedKey)] = new HashTableEntry<>(key, value);
        }
        if (previous == null || !previous.key().equals(key)) {
            ++this.size;
        }
    ...
    private int linearProbedHashedKey(int hashedKey) {
    ...
    public void remove(K key) {
        ...
        int originalHashedKey = hashedKey;
        while (entry == null || !entry.key().equals(key)) {
            hashedKey = nextLinearProbedHashedKey(hashedKey);
            entry = this.entries[hashedKey];
        }
        this.entries[hashedKey] = null;
        removeLinearlyProbed(originalHashedKey, hashedKey);
        --this.size;
    }

    private void removeLinearlyProbed(int originalHashedKey, int removedHashedKey) {
        int previousHashedKey = removedHashedKey;
        int nextHashedKey = nextLinearProbedHashedKey(previousHashedKey);
        HashTableEntry<K, V> entry = this.entries[nextHashedKey];
        while (entry != null) {
            if (hashedKey(entry.key()) == originalHashedKey) {
                this.entries[previousHashedKey] = entry;
            }
            previousHashedKey = nextHashedKey;
            nextHashedKey = nextLinearProbedHashedKey(previousHashedKey);
            entry = this.entries[nextHashedKey];
        }
    }
    ...
    private HashTableEntry<K, V>[] valuedEntries() {
        HashTableEntry<K, V>[] valuedEntries = (HashTableEntry<K, V>[]) Array.newInstance(HashTableEntry.class, this.size);
